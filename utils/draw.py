...
def draw_boxes(img, bbox, identities=None, offset=(0,0), confs=None, target_id=None, target_xyz=None, cls_names=None,
    ...
    for i,box in enumerate(bbox):
        ...
        cv2.putText(img,label,(x1,y1+t_size[1]+4), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
        if confs:
            ...
            cv2.putText(img, '{}{:.3f}'.format("conf:", confs[i]), (x1, y2 + 2 * t_size[1]), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
            cv2.putText(img, '{}'.format(cls_names[i]), (x1, y2 + 4 * t_size[1]), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
            cv2.putText(img, '({:.2f}, {:.2f}, {:.2f})[m]'.format(target_xyz[i][0], target_xyz[i][1], target_xyz[i][2]),
                        (x1, y2 + 6 * t_size[1]), cv2.FONT_HERSHEY_PLAIN, 2, color, 2)
